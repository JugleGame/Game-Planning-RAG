...
updated = "2026-07-30"
confidence = "high" # prompts/5_developer.md 기준 구조의 World_Base UI 명시 + Unity 공식 UI 최적화 지침(정적/동적 캔버스 분리, 리빌드 비용) 근거 + 안티패턴(단일 거대 캔버스) 실사례
+++ 
## 문제
...
- UI 루트는 World_Base 씬에 있고 항상 켜져 있다. Chunk 씬에는 UI를 넣지 않는다. [출처: prompts/5_developer.md 기준 구조 — World_Base.unity "플레이어, 카메라, UI (항상 켜짐)"]
...
- UI는 데이터를 요청하지 않고 방송을 듣는다. 체력 변화, 아이템 획득, 대화 시작은 EventBus 방송을 구독해 갱신한다. UI가 플레이어나 매니저를 직접 참조하지 않는다. [출처: prompts/5_developer.md 방송 규칙]
- 월드 위에 떠 있는 표시(상호작용 안내, NPC 말풍선)도 청크가 아니라 World_Base UI에 둔다. 대상의 위치만 따라가게 하고 UI 자체는 청크와 함께 꺼지지 않게 한다.

## 핵심 규칙

- UI 오브젝트는 World_Base 씬에만 둔다. Chunk 씬에 UI를 넣으면 청크가 꺼질 때 화면 일부가 사라진다. [출처: prompts/5_developer.md 청크 규칙]
...
- UI는 EventBus 구독으로만 갱신한다. 게임플레이 시스템을 매 프레임 들여다보는 방식(폴링)을 쓰지 않는다. [출처: prompts/5_developer.md 방송 규칙]
...
9. 자체 점검 — 컴파일 에러 0개, 콘솔 에러 0개 확인 후 커밋. [출처: prompts/5_developer.md 자체 점검 기준]
...
- UI가 플레이어·매니저를 직접 참조하기: 방송 규칙 위반이며, 청크 로딩으로 참조가 끊기거나 매니저 교체 시 조용히 갱신이 멈춘다. [출처: prompts/5_developer.md 방송 규칙]
...
- 구독 누적 검사: UI 오브젝트를 여러 번 껐다 켠 뒤 이벤트 하나를 발생시켰을 때, 갱신이 한 번만 일어나고 `Logs/commentator.log`에 중복 줄이 없어야 한다. [출처: prompts/5_developer.md 로그 규칙]
