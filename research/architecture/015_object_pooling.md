...
가비지 컬렉션이 겹쳐 프레임이 튄다 [source: Unity Manual, Pooling and reusing objects,
...
- Unity 6000(2021.3 이상)부터 `UnityEngine.Pool` 네임스페이스에 `ObjectPool<T>`가
  ...
  동작을 정의하는 스택 기반 풀이다 [source: Unity Scripting API, Pool.ObjectPool_1,
...
- 총알처럼 빈번한 스폰 오브젝트를 다수 연속 발생시키는 동안 프로파일러의
  GC Alloc이 프레임당 0에 가까운지 확인한다 [source: Unity Manual, Pooling and
  reusing objects].
...
- ELEM-004 (반복 메커닉): 로그라이크형 전투처럼 짧은 반복 루프가 많은 장르일수록
  풀링의 성능 이득이 커진다.
- ELEM-031 (시각 피드백 과장): 이 요소의 구현 전제. 파티클·이펙트를 겹겹이 쌓는
  연출은 풀링 없이는 프레임 비용으로 되돌아온다.
- ELEM-022 (지수적 점수 스케일링): 화면의 동시 오브젝트 수가 후반부에 기하급수적으로
  늘어나는 설계에서는 풀 크기 산정이 곧 성능 상한이 된다.
