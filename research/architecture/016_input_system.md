...
- Unity의 Input System 패키지는 `InputActionAsset` 자산 하나에 "행동(Action)"을
  ...
  [출처: Unity Input System Manual, Input Action Assets,
...
- `PlayerInput` 컴포넌트를 플레이어 오브젝트에 붙이고 이 Action Asset을 연결하면,
  행동이 발생할 때 지정한 메서드를 호출해준다 [출처: Unity Input System Manual,
...
- 2D 이동처럼 상하좌우 네 방향을 하나의 벡터로 합쳐야 하는 입력은 "2D Vector
  Composite"로 구성한다 [출처: Unity Input System Manual, Quick start guide,
