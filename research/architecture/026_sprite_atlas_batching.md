...
- 아틀라스는 여러 스프라이트를 하나의 텍스처로 묶은 자산이다. 같은 아틀라스에 든 스프라이트는
  하나의 그리기 요청으로 함께 처리될 수 있다 [출처: Unity 매뉴얼 'Sprite atlases', 2026-08 확인].
...
- 기기 성능에 따라 해상도를 달리해야 하면 같은 아틀라스의 변형(variant)을 두고 실행 시점에
  적절한 쪽을 고른다 [출처: game-developers.org 'What Is Sprite Atlas in Unity', 2026-08 확인].
- [해석] 아틀라스는 성능 도구이기 이전에 자산 묶음 규약이다. 새 스프라이트를 어느 아틀라스에
...
- [해석] 최적화 판단은 측정 뒤에 한다. 아틀라스를 나눌지 합칠지는 프로파일러의 배칭 수치를
...
- 콘솔 검사: 아틀라스 적용 후 정상 플레이 중 콘솔 에러 0개 [출처: reference/unity_project_baseline.md 4절 자체 점검].
