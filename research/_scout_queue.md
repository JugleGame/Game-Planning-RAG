...
## 2026-08-12 23:05 생성 (카테고리: GAME)
- [ ] Heritage (라이프코스형 솔로 PvE 오토배틀러) — connects_to: GENRE-037 — why_now: 08-12 다이제스트에 따르면 Heritage가 2026-08-11 Steam 출시, 매년 3개 두루마리 선택으로 캐릭터가 성장하고 전투는 축적된 속성·스킬·장비·인생 선택에 따라 완전 자동으로 흘러가는 라이프코스형 구조를 씀 - GENRE-037(솔로 PvE 로그라이크 오토배틀러)의 구체 실증 사례이면서 웨이브 아레나가 아닌 생애주기 프레이밍이라는 변주를 보여주나 아직 개별 GAME 카드가 없음 — obscurity: 높음
- [ ] Dorts: A Darts Roguelike — connects_to: ELEM-021 — why_now: 08-12 다이제스트에 따르면 Dorts가 실제 다트 게임 501 규칙에서 시작해 드래프트 업그레이드와 변화하는 레그 규칙을 얹은 10~15분 로그라이크로 2026-08-12 Steam 출시 예정 - ELEM-021(친숙한 규칙 차용)의 대상이 카드·카지노·도미노·마작을 넘어 실물 정밀 스포츠 규칙으로 확장된 사례이나 아직 개별 GAME 카드가 없음 — obscurity: 높음
- [ ] Crown Jewels — connects_to: ELEM-021 — why_now: 08-12 다이제스트에 따르면 Crown Jewels가 2026-08-10 Steam 출시, 플레이어가 커스텀 6면 주사위를 직접 만들어 보석을 왕관까지 옮기는 2~4인 턴제 디지털 보드게임 - ELEM-021이 다루는 친숙한 규칙 차용의 대상이 주사위·보드게임 관습으로 확장되면서도 주사위 자체를 변형한다는 점에서 구분되는 사례이나 아직 개별 GAME 카드가 없음 — obscurity: 높음
- [ ] corectl — connects_to: GENRE-010, ELEM-018 — why_now: 08-09 다이제스트에 따르면 corectl이 2026-08-05 Steam 출시, 6개 건물 한정 로드아웃과 플레이어가 직접 짜는 적 경로, 웨이브마다 하나의 증강 선택, 영구 연구 효과를 결합한 2D 로그라이트 TD 구조를 씀 - GENRE-010이 무작위 타워 보상을 넘어 경로 설계·로드아웃 제한까지 런 단위 결정으로 확장되는 흐름의 구체 사례이나 아직 개별 GAME 카드가 없음 — obscurity: 중간
- [ ] Anomaly President — connects_to: ELEM-018 — why_now: 08-09 다이제스트에 따르면 Anomaly President가 2026-08-03 Steam 얼리액세스 출시, 낮에는 선거 유세·팀 관리를, 밤에는 액션 전투를 번갈아 진행하며 로그라이크 업그레이드가 매 런을 형성하는 구조를 씀 - ELEM-018(로그라이크 무작위 업그레이드/경로 드래프트)이 정치 관리·기지 건설이라는 새로운 프레임에 결합되는 사례이며, 이 조합을 정확히 담는 GENRE 카드가 레지스트리에 없어 우선 GAME 카드로 기록해 둘 필요 — obscurity: 중간

## 2026-08-11 23:05 생성 (카테고리: ELEM)
- [ ] 코어 스탯 배율 하향 리밸런싱 (Core Stat Multiplier Nerf Rebalancing) — connects_to: ELEM-022, GAME-023 — why_now: Diablo IV 3.2.0 PTR이 힘(Strength) 10당 대미지 증가율을 1.1%→0.8%로 낮추는 코어 스탯 스케일러 조정을 테스트 중(08-07 다이제스트) - ELEM-022는 곱연산이 쌓여 파워가 폭발적으로 늘어나는 상승 방향만 다루고, 코어 스탯 자체의 곱연산 계수를 낮춰 파워 인플레를 억제하는 하향 리밸런싱은 별도 요소로 기록된 적 없음 — obscurity: 낮음
- [done] 신격 선택형 소환 로스터 고정 (Deity/Patron Selection Locking Summonable Roster) — connects_to: GENRE-010, ELEM-036 — why_now: 08-07 다이제스트에서 확인된 그리스 신화 로그라이트 TD 'Gods & Gore'가 시작 시 고른 신에 따라 이후 소환 가능한 신화 생물군 전체가 고정되는 구조를 씀 - ELEM-036(픽/밴 드래프트)은 경쟁 매치 시작 전 영웅 개체를 고르는 절차만 다루고, PvE에서 '고른 후원자/클래스가 이후 유닛 풀 전체를 제한'하는 구조는 별도 요소로 기록된 적 없음 — obscurity: 높음 — ELEM-045
- [done] 전투 참여형 동물 동료 자동 보조 (Combat-participating Animal Companion Auto-assist) — connects_to: ELEM-008 — why_now: Beast of Reincarnation(2026-08-04 출시)이 단일 캐릭터+동물 동료(Koo) 구조로, 동료가 서사적 반응이 아니라 전투 중 자동으로 딜/보조를 넣는 방식을 씀(08-07 다이제스트) - ELEM-008은 곁에서 반응해주는 동반자 일반을 다룰 뿐, 별도 조작 없이 전투에 자동 참여하는 동료 구조는 분리된 요소로 기록된 적 없음 — obscurity: 높음 — ELEM-046
- [ ] 무료 콘텐츠 업데이트를 통한 완제품 인디게임 수명 연장 (Post-launch Free Content Updates Sustaining Premium Indie Longevity) — connects_to: GAME-031, ELEM-032 — why_now: Balatro Wiki 기준 2024년 출시작이 2년 넘게 무료 업데이트("Friends of Jimbo 4" 등)로 라이브 지원을 지속하며 "Balatro-like"가 하위장르 용어로 정착(07-31 다이제스트) - ELEM-032(뉴 게임 플러스)는 유저가 자발적으로 재도전하는 재플레이 장치만 다루고, 개발사가 무료 콘텐츠를 계속 추가해 완제품 게임의 활성 플레이어를 유지하는 퍼블리싱 전략은 별도 요소로 기록된 적 없음 — obscurity: 낮음
- [done] 비대칭 시작덱 캐릭터 로스터 (Asymmetric Starting-deck Character Roster) — connects_to: ELEM-020, GENRE-012 — why_now: 08-07 다이제스트의 Rogueliker.com 8월 월간 정리에 일본 신화(요괴) 소재 로그라이크 덱빌더가 3개 플레이어블 캐릭터로 PC·Switch 동시 출시된다고 언급됨 - ELEM-020(덱 구축)은 단일 덱이 자라나는 구조만 다루고, 캐릭터마다 서로 다른 시작 덱/키트를 부여해 재플레이 가치를 만드는 로스터 구조는 별도 요소로 기록된 적 없음 — obscurity: 중간 — ELEM-047
...
- [done] Dominocalypse (도미노 타일매칭 로그라이크 덱빌더) — connects_to: GENRE-035, ELEM-021 — why_now: GENRE-035(타일매칭 로그라이크 덱빌더, 08-09 신설)의 신설 근거가 된 게임 자체가 아직 개별 GAME 카드로 없음 - 친숙한 규칙 차용(ELEM-021) 대상이 포커류를 넘어 도미노로 확장된 구체 사례 — obscurity: 높음 — GAME-054
- [done] Loop Hero (2021, Four Quarters / Devolver Digital) — connects_to: GENRE-037, ELEM-004 — why_now: GENRE-037(솔로 PvE 로그라이크 오토배틀러, 08-09 신설)이 Backpack Battles류로만 예시돼 있고 이를 실증할 GAME 카드가 없음 - 카드를 루프 경로에 배치하면 전투 자체는 완전 자동으로 흘러가는 원형적 사례 — obscurity: 중간 — GAME-055
- [done] Factorio (2020, Wube Software) — connects_to: GENRE-036, ELEM-039 — why_now: GENRE-036(팩토리 자동화 빌더, 08-09 신설, "생물 포획 없이 순수 생산 사슬"로 정의)를 실증할 GAME 카드가 없음 - 장르명 자체가 유래한 원형적 사례로, GENRE-030(크리처 노동 자동화)의 Palworld와 대비되는 짝 사례 — obscurity: 낮음 — GAME-056
