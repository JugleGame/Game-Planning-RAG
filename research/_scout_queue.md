...
- [x] Dominocalypse (도미노 타일매칭 로그라이크 덱빌더) — connects_to: GENRE-035, ELEM-021 — why_now: GENRE-035(타일매칭 로그라이크 덱빌더, 08-09 신설)의 신설 근거가 된 게임 자체가 아직 개별 GAME 카드로 없음 - 친숙한 규칙 차용(ELEM-021) 대상이 포커류를 넘어 도미노로 확장된 구체 사례 — obscurity: 높음
- [x] Loop Hero (2021, Four Quarters / Devolver Digital) — connects_to: GENRE-037, ELEM-004 — why_now: GENRE-037(솔로 PvE 로그라이크 오토배틀러, 08-09 신설)이 Backpack Battles류로만 예시돼 있고 이를 실증할 GAME 카드가 없음 - 카드를 루프 경로에 배치하면 전투 자체는 완전 자동으로 흘러가는 원형적 사례 — obscurity: 중간
- [x] Factorio (2020, Wube Software) — connects_to: GENRE-036, ELEM-039 — why_now: GENRE-036(팩토리 자동화 빌더, 08-09 신설, "생물 포획 없이 순수 생산 사슬"로 정의)를 실증할 GAME 카드가 없음 - 장르명 자체가 유래한 원형적 사례로, GENRE-030(크리처 노동 자동화)의 Palworld와 대비되는 짝 사례 — obscurity: 낮음
