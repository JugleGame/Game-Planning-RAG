...
- [x] 신격 선택형 소환 로스터 고정 (Deity/Patron Selection Locking Summonable Roster) — connects_to: GENRE-010, ELEM-036 — why_now: 08-07 다이제스트에서 확인된 그리스 신화 로그라이트 TD 'Gods & Gore'가 시작 시 고른 신에 따라 이후 소환 가능한 신화 생물군 전체가 고정되는 구조를 씀 - ELEM-036(픽/밴 드래프트)은 경쟁 매치 시작 전 영웅 개체를 고르는 절차만 다루고, PvE에서 '고른 후원자/클래스가 이후 유닛 풀 전체를 제한'하는 구조는 별도 요소로 기록된 적 없음 — obscurity: 높음
- [x] 전투 참여형 동물 동료 자동 보조 (Combat-participating Animal Companion Auto-assist) — connects_to: ELEM-008 — why_now: Beast of Reincarnation(2026-08-04 출시)이 단일 캐릭터+동물 동료(Koo) 구조로, 동료가 서사적 반응이 아니라 전투 중 자동으로 딜/보조를 넣는 방식을 씀(08-07 다이제스트) - ELEM-008은 곁에서 반응해주는 동반자 일반을 다룰 뿐, 별도 조작 없이 전투에 자동 참여하는 동료 구조는 분리된 요소로 기록된 적 없음 — obscurity: 높음
