...
- 핵심 루프: 플레이어의 조작 없이(또는 탭 한두 번으로) 자원/점수가 시간에 비례해 계속 쌓이는 것이 코어 동사다.
  ...
  구분 짓는 가장 핵심적인 장치다.
- ELEM-022 (지수적 점수 스케일링)가 장르의 수학적 엔진 역할을 한다. [해석] 이 장르에서 ELEM-022는
...
- 프레스티지/환생(승천) 루프: 특정 지점에서 진행을 초기화하는 대신 영구 배율(곱연산 보너스)을 얻는
  메타 순환이 관례적으로 따라붙는다(Cookie Clicker의 ascension류). [해석] 이는 지수 곡선이 화면 표시
...
- AFK Arena(Lilith Games)는 누적 다운로드 4,500만 건 이상, 전체 누적 매출 약 10억 달러를 기록했다
  [출처: Sensor Tower 데이터 인용, Udonis 블로그 "AFK Arena Analysis", 2026 확인]. 다만 최근 월간
  지표(미국 앱스토어 기준 월 4만 다운로드·60만 달러, 구글플레이 월 6만 다운로드·60만 달러 수준)는
  2020년 1월 정점 대비 크게 낮아진 상태다 [출처: 같은 자료].
- Sensor Tower가 "Idle RPG"를 별도 카테고리로 상시 집계·발표한다는 사실 자체가, 이 장르(정확히는
  캐릭터 수집+자동 전투가 결합한 하위장르)가 시장에서 독립 세그먼트로 인정받고 있다는 근거다
  [출처: Sensor Tower blog, "Top 5 Idle RPGs" 시리즈, 2024~2025년 게시분].
- StoneAge: Idle Adventure는 2026년 3월 전 세계 매출 성장 기준 4위를 기록했다 [출처: Sensor Tower,
  "Top 10 Worldwide Mobile Games" 2026년 3월 기준].
- 2026년 상반기 글로벌 모바일 게임 인앱결제(IAP) 매출은 400억 달러로 전년 대비 2% 감소했고, 광고
  매출 비중이 커지는 추세다 [출처: Sensor Tower H1 2026 리포트 인용, Prism News, 2026 확인].
  [해석] 이는 방치형 장르 고유의 쇠퇴라기보다 모바일 게임 시장 전반의 IAP 의존도 하락으로 보이며,
  이 장르만의 특이 신호로 단정하기는 어렵다.
- PC/웹 사례: Cookie Clicker의 Steam 버전 누적 매출은 약 2,190만 달러로 추정된다
  [출처: Steam Revenue Calculator(제3자 추정 도구, 방법론 비공개), 2026-08 확인 — 공식 발표 수치가
  아니므로 참고용]. 모바일 파생작 "Cookies Inc. - Idle Clicker"는 240만 다운로드를 기록했다
  [출처: AppBrain, 2026-08 확인].
<!-- 증거 부족: Egg Inc의 구체적 매출·다운로드 수치는 이번 검색에서 확인하지 못함. 결과에 장르
     설명만 나오고 정량 데이터가 없었음 -->
- [해석] 상위권 방치형/증분형 게임 다수가 "Idle RPG"(캐릭터 수집 + 자동 전투) 형태로 수렴하고 있어,
...
- 오프라인 누적 자원에 상한을 두지 않거나(무한 누적), 시간당 상한(캡)을 두는 두 갈래 설계가
  공존한다. [해석] 캡이 있으면 "너무 오래 방치하면 오히려 손해"라는 역설적 압박이 생겨, 순수
...
- 광고 시청을 통한 보상 배율 부스트("광고 보고 2배 받기")가 수익화의 기본값으로 자리잡았다.
  [해석] 시장 포화도 항목의 광고 매출 비중 증가 추세와 맞물려, 방치형 장르가 광고 기반 수익화
  실험의 최전선 중 하나가 되고 있을 가능성이 있다.
- ELEM-022와 결합할 때 화면에 표시되는 숫자가 후반부로 갈수록 과학적 표기(예: 1.2e15)로 전환되는
  것이 관례다. [해석] 이는 ELEM-022 카드가 지적한 "지수 증가는 UI 표시 부담을 만든다"는 리스크가,
...
## 빈칸
- [해석] Sensor Tower가 별도로 "Idle RPG"를 집계할 만큼 캐릭터 수집(가챠) 결합형 하위장르는 이미
